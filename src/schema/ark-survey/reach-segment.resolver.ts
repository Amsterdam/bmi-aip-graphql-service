...
export class ReachSegmentResolver {
	...
	@Roles({ roles: ['realm:aip_owner', 'realm:aip_admin', 'realm:aip_survey'], mode: RoleMatchingMode.ANY })
	async arkSurveyReachSegments(@Args('arkSurveyId', { type: () => String }) arkSurveyId: string) {
		return this.queryBus.execute<FindReachSegmentsQuery>(new FindReachSegmentsQuery(arkSurveyId));
	}
}
